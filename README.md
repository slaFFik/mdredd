...
├── transcript.ndjson            ← append-only normalized log, written as the run streams
├── transcript.json              ← normalized event log, finalized at run end
...
- **Your user-global setup, but only when you switch the topbar "User scope" toggle on.** Off (the default) keeps variants project-scoped: no `~/.claude/CLAUDE.md`, no user-level skills, plugins, or slash commands. Turning it on widens the child's setting sources to `user,project`, so those load the way they would in a real session on your machine.
...
- A baseline ~5–10k cache-creation tokens still come from Claude Code's own system prompt and tool schemas, plus your user-global config (`~/.claude/CLAUDE.md`, user-level skills) when the User scope toggle is on. That overhead is the same for every variant in a session, so it cancels out in A/B comparisons, but it's not zero.
