...
1. **Preflight** (`src/server/preflight.ts`) — verifies the `claude` binary is on PATH, has required CLI flags, and is authenticated (a quick `-p ping` smoke test using `--json-schema`). Acquires `~/.mdredd/.lock` via `proper-lockfile` (single-instance guarantee); writes a `.lock.meta.json` sidecar with pid/port for human debugging. Mass-marks any `preparing`/`streaming` runs from a previous boot as `abandoned` and ensures a `~/.mdredd/.gitignore` exists.
...
6. **Judge** (`src/server/judge.ts`) — subprocess Haiku invocation with `--json-schema`. Each transcript section is byte-capped (`JUDGE_*_CAP_*` in `src/shared/constants.ts`). On timeout, retries once with halved caps (issue #12). A per-run **canary token** is embedded in the prompt; if the judge's response contains it, the run is treated as poisoned by prompt injection and rejected.
